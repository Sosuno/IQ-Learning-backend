...
drop function fuck_hibernate(a article);
drop type article;
CREATE TYPE article AS (
            ...
            edited_on TIMESTAMP,
            upvoted_by BIGINT ARRAY ,
            tags BIGINT ARRAY ,
            image bytea
);


CREATE OR REPLACE FUNCTION fuck_hibernate(a article) RETURNS ARTICLE AS '
  DECLARE
    newId bigint;
    rec record;
   BEGIN
    INSERT INTO articles VALUES (a.owner,a.title,a.contents,a.tags) returning id into newId;
    EXECUTE ''SELECT * FROM articles WHERE id = newId'' into rec;
    RETURN rec;
  END;
'LANGUAGE plpgsql;
